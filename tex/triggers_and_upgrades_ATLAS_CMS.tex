...
%The striking signatures of long-lived particles present unique experimental challenges in triggering, signal reconstruction, and background estimation. 
...
In term of ATLAS upgrade, the standard analysis could be improved by using  HCal information splitting the B-C layers to identify the scalar decay position.
...
For the CMS MTD located just outside the tracker volume,  $ {\ell_{\rm SM}}/{\beta_{\rm SM}}$ is about $O(1ns)$. 
...
%The small DM halo velocities imply that DM has insufficient energy to up-scatter into the heavier state, and so interactions through the off-diagonal coupling are ineffective today.
