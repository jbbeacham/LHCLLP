\noindent {\bf Chapter editors:}~Giovanna Cottin, Nishita Desai, Sabine Kraml, Andre Lessa, Zhen Liu\\
\text{ \; }\\
\noindent {\bf Contributors:}~Juliette Alimena, Will Buttinger, Eric Conte, Yanou Cui, Jared Evans, Benjamin Fuks, Lukas Heinrich, Jan Heisig, Gavin Hesketh, David Michael Morse, Michael Ramsey-Musolf, Ennio Salvioni, Michele Selvaggi, Brian Shuve, Yuhsin Tsai
\text{ \; }\\
...
axinos~\cite{Pagels:1981ke,Covi:1999ty}, or $R$-parity violation~\cite{Barbier:2004ez,LopezFogliani:2005yw,Ghosh:2017yeh},
...
reinterpret the LLP experimental results
...
are publicly available and allow the user to reinterpret SUSY simplified-model results within the context of a full model.
...
parameters required for a general purpose reinterpretation.
...
Second, recasting LLP searches outside of the experimental collaborations is a
...
As a result, while first steps towards the treatment of LLPs were achieved in \textsc{SModelS}~\cite{Heisig:2018kfq} and \textsc{MadAnalysis5}~\cite{MA5:longlivedleptons}, the implementation of LLP searches in public recasting tools is still in its infancy, thus limiting the applicability of the experimental results.

In order to allow for a more extensive reinterpretation or recasting of
...
for presenting the LLP results (and for a summary, see Figure~\ref{fig:ch5-complexity-vs-modelindependence}), and compare their advantages and
shortcomings.
In Section~\ref{sec:ch5-smsReinterpretations}, we discuss in more details how the simplified models defined in Section~\ref{sec:simplifiedmodel} can be used to reinterpret LLP searches.
...
In Section~\ref{sec:ch5-recastingPrompt}, we discuss complementary constraints on LLPs from reinterpreting prompt searches.
...
Figure~\ref{fig:ch5-complexity-vs-modelindependence}.
...
Figure~\ref{fig:ch5_presentation_options}.
...
rarely be used for reinterpretation without significant effort;
...
constraints to complex models, where multiple topologies are present~\footnote{These points are illustrated by the reinterpretation of the CMS search
