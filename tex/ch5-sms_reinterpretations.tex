...
prompt and LLP searches. As discussed in {\color{blue}{Chapter~\ref{whitepaper:simplified-models}}}, a large
...
% {\color{red}{GC: We also intend to show in this section the displaced vertex recast for one of the Simplified Model from the library, described in Table 9 of the Simplified Models chapter. In this way, we make use of the library
% that's being put together and also can serve as a way of showing how best to present results for a given simplified model (i.e efficiencies for different lifetimes, LLP masses, etc.). One could then do similar exercise, as presented for the HSCPs in  this subsection, to go from these SMS results to a full model. Unless people think including this would be unnecessary, given that we are already highlighting the connection with simplified models?}}
...
a new parameter must be considered: the LLP lifetime (see {\color{blue}{Ch.~\ref{whitepaper:simplified-models}}}).
